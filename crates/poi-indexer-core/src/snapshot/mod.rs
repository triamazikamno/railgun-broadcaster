...
mod tests {
    ...
    fn snapshot_roundtrip_preserves_events_and_excludes_blocked_shields() {
        ...
        let events = vec![
            stored_event(0, 1, PoiEventType::Shield),
            stored_event(1, 2, PoiEventType::Transact),
        ];
    ...
    fn snapshot_reader_rejects_corrupt_magic() {
        let events = vec![stored_event(0, 1, PoiEventType::Shield)];
    ...
    fn stored_event(event_index: u64, byte: u8, event_type: PoiEventType) -> StoredEvent {
        StoredEvent {
            event_index,
