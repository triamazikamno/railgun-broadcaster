...
fn canonical_poi_event_message(event: &SignedPoiEvent) -> Vec<u8> {
    format!(
...
fn canonical_blocked_shield_message(record: &SignedBlockedShield) -> Vec<u8> {
    ...
    if let Some(reason) = &record.block_reason {
        ...
        message.push_str(&serde_json::to_string(reason).expect("string serialization cannot fail"));
    }
    message.push('}');
    message.into_bytes()
}
...
mod tests {
    ...
    fn verifies_blocked_shield_golden_vectors() {
        ...
        // The live upstream currently returns a blockReason for every blocked shield.
        verify_blocked_shield(
